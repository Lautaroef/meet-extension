// live-meeting-assistant/webapp/src/lib/logger.ts
...
const formatMessage = (level: string, message: string, context?: LogContext): string => {
  let logEntry = `${getTimestamp()} [${level.toUpperCase()}] ${message}`;
  if (context && Object.keys(context).length > 0) {
    try {
      logEntry += ` | Context: ${JSON.stringify(context)}`;
    } catch (_e) {
      void _e; // Explicitly mark as unused
...
const logger = {
  debug: (message: string, context?: LogContext): void => {
    if (process.env.NODE_ENV !== 'production' || process.env.NEXT_PUBLIC_DEBUG_LOGS === 'true') {
